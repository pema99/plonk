...
Operators are subject to change, and more will probably be added. The library includes a few common parser combinato data sources, such as one for parsing text while keeping track of line and column number, and one for parsing arbitrary sequences. These can be found in `src/Common.fs`.
